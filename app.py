...
@app.callback(
    ...
    [Input('run-button', 'n_clicks'),
     ...
     Input('anti', 'value'),
     # 特殊规则
     Input('reroll_hits_failed', 'value'),
     Input('reroll_wounds_failed', 'value'),
     # 暴击修正
     Input('critical_hit_value', 'value'),
     Input('critical_wound_value', 'value')
     ],  # 改变这里
...
def update_graph(n_clicks,
    ...
    dice_params = DiceParameters(
        ...
        anti=anti,
        reroll_hits_failed=reroll_hits_failed,
        reroll_wounds_failed=reroll_wounds_failed,
        critical_hit_value=critical_hit_value,
        critical_wound_value=critical_wound_value,
...
@app.callback(
    [Output('attack_input', 'value'),
     ...
     Output('anti', 'value'),
     Output('hit_modify', 'value'),
     Output('wound_modify', 'value'),
     Output('armor_modify', 'value'),
     # 特殊规则
     Output('reroll_hits_failed', 'value'),
     Output('reroll_wounds_failed', 'value'),
     # 暴击修正
     Output('critical_hit_value', 'value'),
     Output('critical_wound_value', 'value')
     ],
...
def reset_values(n):
    if n is None:
        ...
        return '', 4, 4, 4, 7, '', False, False, False, False, False, 0, False, False, 7, 0, 0, 0, False, False, 6, 6
        # 攻击次数，命中，造伤，保护，不怕疼，伤害数值，洪流，重投命中，重投造伤，重投命中1，重投造伤1，连击，毁灭伤害，致命一击，针对, 命中修正，造伤修正，保护修正。
